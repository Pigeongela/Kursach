...
namespace Kursach
{
    public partial class Form1 : Form
    {
        ...
        string[] f_name = {"cos(x)", "sin(x)", "a^x", "e^x", "x^n"};//список функций
        LineItem myCurve;
        ...
        public void Func1(double a, double b, int i, int n, double k)
        {
        ...
        public void Func2(double a, double b, int i, int n, double k)
        {
            //Получим панель для рисования
            GraphPane pane = zedGraphControl1.GraphPane;
            //Очистим список кривых на тот случай, если до этого сигналы уже были нарисованы
            //pane.CurveList.Clear();
            //Создадим список точек
            PointPairList ff_list = new PointPairList();//для графика функции
            ...
            {
                // добавим в список точку
                ff_list.Add(x, k * f(x, i, n));
            }

            //Создадим кривую с названием выбранного графика, которая будет рисоваться голубым цветом, выделим опорные точки окружностями
            myCurve = pane.AddCurve(f_name[i], ff_list, Color.Blue, SymbolType.Circle);
        ...
        public void DrawRect(double a, double b, int i, int n, double k) //метод серединных прямоугольников
        {
            ...
            myCurve2.Line.Fill = new ZedGraph.Fill(Color.FromArgb(rnd.Next(256)*i, rnd.Next(256), rnd.Next(256)));
        ...
        public void DrawTrap(double a, double b, int i, int n, double k) //метод трапеций
        {
            ...
            myCurve2.Line.Fill = new ZedGraph.Fill(Color.FromArgb(rnd.Next(256), rnd.Next(256)*i, rnd.Next(256)));
            zedGraphControl1.AxisChange();
            zedGraphControl1.Invalidate();
        }
